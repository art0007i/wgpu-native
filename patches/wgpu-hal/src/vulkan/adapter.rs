...
impl PhysicalDeviceProperties {
    ...
    fn get_required_extensions(&self, requested_features: wgt::Features) -> Vec<&'static CStr> {
        ...
        // Always enable `VK_KHR_get_memory_requirements2`. On Vulkan 1.1+ it is promoted to core
        // so the driver may not list it in vkEnumerateDeviceExtensionProperties, but the Vulkan
        // spec allows enabling promoted extensions without enumeration when apiVersion >= 1.1.
        // Some OpenXR runtimes (e.g. SteamVR) call vkGetDeviceProcAddr("vkGetImageMemoryRequirements2KHR")
        // and crash if the Vulkan loader cannot alias the KHR name, which requires the extension
        // to be present in ppEnabledExtensionNames at device creation.
        if self.device_api_version >= vk::API_VERSION_1_1
            || self.supports_extension(khr::get_memory_requirements2::NAME)
        {
